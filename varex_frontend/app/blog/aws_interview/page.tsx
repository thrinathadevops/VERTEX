// PATH: app/blog/aws_interview/page.tsx
"use client";
...
import { Calendar, Clock3, Lock, BookOpen } from "lucide-react";

export default function AWSInterviewBlogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "aws_interview"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <nav className="flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-300">AWS Interview</span>
      </nav>

      <header className="space-y-2">
        <div className="flex items-center gap-2">
          <BookOpen className="h-7 w-7 text-teal-300" />
          <h1 className="text-2xl font-bold">AWS Interview Prep</h1>
        </div>
        <p className="text-sm text-slate-300 max-w-lg">Enterprise-grade AWS interview questions & answers — direct from our GitHub repository.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <BookOpen className="h-8 w-8 text-teal-300 mx-auto" />
