// PATH: app/blog/architecture/page.tsx
"use client";
...
import { Calendar, Clock3, Lock, Waypoints } from "lucide-react";
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "architecture"));
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <header className="space-y-2">
        <div className="flex items-center gap-2">
          <Waypoints className="h-7 w-7 text-purple-300" />
          <h1 className="text-2xl font-bold">Architecture</h1>
        </div>
        <p className="text-sm text-slate-300 max-w-lg">System design, cloud architecture, microservices, and scalability patterns.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <Waypoints className="h-8 w-8 text-purple-300 mx-auto" />
