// PATH: app/learnings/page.tsx
"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import ProtectedRoute from "@/components/ProtectedRoute";
import { getMe, listPremiumContent, listFreeContent } from "@/lib/api";
...
function LearningsInner() {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      {/* Premium user — full content grid */}
      {isPremium && items.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-slate-200">All modules ({items.length})</h2>
                ...
                <article className="group rounded-2xl border border-slate-800 bg-slate-900/70
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className="text-sm font-semibold text-slate-100 line-clamp-2
                      group-hover:text-sky-300 transition">{item.title}</h3>
          ...
          <div className="rounded-2xl border border-sky-700/70 bg-sky-950/40 p-5">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="rounded-full bg-amber-500/20 px-2 py-0.5
                text-[10px] font-medium text-amber-200">Recommended</span>
            </div>
            ...
            <button onClick={handleUpgrade} disabled={paying}
              ...
              {paying
                ? "Opening payment..."
                : `Pay · ${billing === "monthly" ? "₹1,499/mo" : "₹3,999/qtr"}`}
            </button>
              ...
              <Link key={item.id} href={`/learnings/${item.slug ?? item.id}`}>
                <article className="group rounded-2xl border border-slate-800 bg-slate-900/70
                  ...
                  <h3 className="text-sm font-semibold text-slate-100 line-clamp-2 mb-1
                    group-hover:text-sky-300 transition">{item.title}</h3>
                  <p className="text-xs text-slate-400 line-clamp-2 blur-[2px]
                    group-hover:blur-none transition">
                    {item.body?.replace(/<[^>]*>/g, "").slice(0, 120)}...
                  </p>
...
export default function LearningsPage() {
  return <ProtectedRoute><LearningsInner /></ProtectedRoute>;
}
