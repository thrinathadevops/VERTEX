// PATH: app/blog/linux_interview/page.tsx
"use client";
...
import { Calendar, Clock3, Lock, Terminal } from "lucide-react";

export default function LinuxInterviewBlogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "linux_interview"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <nav className="flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-300">Linux Interview</span>
      </nav>

      <header className="space-y-2">
        <div className="flex items-center gap-2">
          <Terminal className="h-7 w-7 text-emerald-300" />
          <h1 className="text-2xl font-bold">Linux Interview Prep</h1>
        </div>
        <p className="text-sm text-slate-300 max-w-lg">Enterprise-grade Linux interview questions & answers — direct from our GitHub repository.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <Terminal className="h-8 w-8 text-emerald-300 mx-auto" />
