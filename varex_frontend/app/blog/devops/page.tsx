...
import { Calendar, Clock3, Cpu, Lock } from "lucide-react";

export default function DevOpsBlogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "devops"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <header className="space-y-2">
        ...
        <p className="text-sm text-slate-300 max-w-lg">CI/CD pipelines, Kubernetes, IaC, GitOps, and SRE practices.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <Cpu className="h-8 w-8 text-sky-300 mx-auto" />
