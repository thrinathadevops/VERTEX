// PATH: app/blog/ai_hiring/page.tsx
"use client";
...
import { Bot, Calendar, Clock3, Lock } from "lucide-react";

export default function AIHiringBlogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "ai_hiring"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <nav className="flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-300">AI Hiring</span>
      </nav>
      ...
      <header className="space-y-2">
        ...
        <p className="text-sm text-slate-300 max-w-lg">AI-powered hiring frameworks, interview prep, and talent acceleration.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <Bot className="h-8 w-8 text-emerald-300 mx-auto" />
