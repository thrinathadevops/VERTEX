// PATH: app/blog/azure_interview/page.tsx
"use client";
...
import { Calendar, Clock3, Lock, Cloud } from "lucide-react";

export default function AzureInterviewBlogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItems(all.filter((item) => item.category === "azure_interview"));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <nav className="flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-300">Azure Interview</span>
      </nav>

      <header className="space-y-2">
        <div className="flex items-center gap-2">
          <Cloud className="h-7 w-7 text-blue-300" />
          <h1 className="text-2xl font-bold">Azure Interview Prep</h1>
        </div>
        <p className="text-sm text-slate-300 max-w-lg">Enterprise-grade Azure interview questions & answers — direct from our GitHub repository.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 space-y-3">
          <Cloud className="h-8 w-8 text-blue-300 mx-auto" />
